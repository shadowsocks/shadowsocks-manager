...
const getFlowFromSplitTime2 = async (serverId, accountId, start, end) => {
  const time = await splitTime(start, end);
  const sum = [];
  let getFlow;
  if(serverId) {
...
const getServerPortFlow2 = async (serverId, accountId, timeArray, isMultiServerFlow) => {
  const result = [];
  timeArray.forEach((time, index) => {
    if(index === timeArray.length - 1) {
      return;
    ...
    const endTime = +timeArray[index + 1];
    let getFlow;
    result.push(getFlowFromSplitTime2(isMultiServerFlow ? 0 : serverId, accountId, startTime, endTime));
  });
  return Promise.all(result);
};
...
const getUserPortLastConnect = async accountId => {
  ...
  //       port: port + server.shift,
  //     });
  //     knex5MinQuery = knex5MinQuery.where({
  //       id: server.id,
  //       port: port + server.shift,
  //     });
  //   } else {
  ...
  //       port: port + server.shift,
  //     });
  //     knex5MinQuery = knex5MinQuery.orWhere({
  //       id: server.id,
  //       port: port + server.shift,
  //     });
  //   }
...
exports.getServerPortFlow2 = getServerPortFlow2;
