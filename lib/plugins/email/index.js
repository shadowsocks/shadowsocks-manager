  ...
  var _ref = _asyncToGenerator(function* (to, subject, text, options = {}) {
    ...
    const checkLimit = (() => {
      var _ref2 = _asyncToGenerator(function* (ip, session) {
        ...
        const sessionNumber = yield knex('email').where({ session }).whereBetween('time', [Date.now() - 3600 * 1000, Date.now()]).count('time as count').then(function (success) {
          return success[0].count;
        });
    ...
    if (number >= 40) {
      return Promise.reject('ip or session send email out of limit');
    }
