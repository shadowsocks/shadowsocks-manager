...
setInterval(_asyncToGenerator(function* () {
  ...
  orders.forEach(function (order) {
    if (order.status !== 'TRADE_SUCCESS' && order.status !== 'FINISH') {
      ...
      account.addAccountLimitToMonth(userId, accountId).then(function () {
...
const orderList = (() => {
  var _ref4 = _asyncToGenerator(function* () {
    const orders = yield knex('alipay').select(['alipay.orderId', 'user.username', 'account_plugin.port', 'alipay.amount', 'alipay.status', 'alipay.alipayData', 'alipay.createTime', 'alipay.expireTime']).leftJoin('user', 'user.id', 'alipay.user').leftJoin('account_plugin', 'account_plugin.id', 'alipay.account').orderBy('alipay.createTime', 'DESC');
