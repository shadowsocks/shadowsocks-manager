...
const getServerFlow = (() => {
  var _ref = _asyncToGenerator(function* (serverId, timeArray) {
    ...
    timeArray.forEach(function (time, index) {
      ...
      const getFlow = knex('saveFlow').sum('flow as sumFlow').groupBy('id').select(['id']).where({ id: serverId }).whereBetween('time', [startTime, endTime]).then(function (success) {
        if (success[0]) {
...
const getlastConnectTime = (serverId, port) => {
  return knex('saveFlow').select(['time']).where({ id: serverId, port }).orderBy('time', 'desc').limit(1).then(success => {
    if (success[0]) {
...
const getServerUserFlow = (serverId, timeArray) => {
  return knex('saveFlow').sum('saveFlow.flow as flow').groupBy('saveFlow.port').select(['saveFlow.port', 'user.userName']).leftJoin('account_plugin', 'account_plugin.port', 'saveFlow.port').leftJoin('user', 'account_plugin.userId', 'user.id').where({
    'saveFlow.id': +serverId
  }).whereBetween('saveFlow.time', timeArray);
};

const getAccountServerFlow = (accountId, timeArray) => {
  return knex('saveFlow').sum('saveFlow.flow as flow').groupBy('saveFlow.id').select(['server.name']).leftJoin('server', 'server.id', 'saveFlow.id').leftJoin('account_plugin', 'account_plugin.port', 'saveFlow.port').where({ 'account_plugin.id': accountId }).whereBetween('saveFlow.time', timeArray);
  ;
};
