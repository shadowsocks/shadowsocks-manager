...
const getFlow = function () {
  if (arguments[3]) {
    ...
    return knex('saveFlow').innerJoin('server', 'server.name', 'saveFlow.name').sum('flow as sumFlow').groupBy('saveFlow.port').select(['saveFlow.port as port']).where({
      'server.host': host,
      'server.port': port
    ...
    return knex('saveFlow').sum('flow as sumFlow').groupBy('port').select(['port']).where({ name }).whereBetween('time', [startTime, endTime]);
  } else {
