...
gulp.task('freeAccountCopy', () => {
  return gulp
    .src(['plugins/freeAccount/libs/**', 'plugins/freeAccount/views/**'], {
      base: './',
    })
    .pipe(gulp.dest('lib'));
});
...
gulp.task(
  'webguiCopy',
  gulp.parallel('webguiBuild', 'webguiLib', () => {
    return gulp
      .src(
        [
          'plugins/webgui/libs/**',
...
gulp.task(
  'babel',
  gulp.parallel('webguiCopy', 'freeAccountCopy', 'babelCopy', () => {
    return gulp
      .src([
        '**/*.js',
        '!node_modules/**',
